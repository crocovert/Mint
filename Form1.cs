...
namespace Mint
{
    public partial class Form1 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            String nom_fichier = saveFileDialog1.FileName;
            nom_fichier = System.IO.Path.GetFullPath(nom_fichier) ;
            int id_bucket = 0;
            List<List<int>> touches = new List<List<int>>();
            Network reseau = new Network();
            Matrix matrice = new Matrix();
            System.IO.File.Delete(nom_fichier + "_det.txt");
            SortedDictionary<ReachLink, int> reached = new SortedDictionary<ReachLink, int>(new ReachLinkComparer());
            reseau=ReadNetworkFromCsv(textBox1.Text);
            matrice = ReadMatrixFromCsv(textBox2.Text);
            ...
            reseau.cboa = float.Parse(textBox6.Text);
            ...
            {
                avance++;
                progressBar1.Value = 100 * avance / matrice.trips.Count;
                textBox7.Text = "Destinations: " + avance + "/" + matrice.trips.Count;
                textBox7.Refresh();
...
//                              MessageBox.Show(id_pivot + " " + cout_max);
                    ...
                    foreach (int link_id in reseau.nodes[reseau.node_num[reseau.links[id_pivot].i]].incoming)
                    {
                        ...
                        //                     PrintStrategies(reseau,fich_stg,list_trip[0]);
                        if ((reseau.links[link_id].i != reseau.links[id_pivot].j) || (reseau.links[link_id].hdwy != 0 || reseau.links[id_pivot].hdwy!=0))
                        {
        ...
        public Network ReadNetworkFromCsv(String filename)
        {
            string chaine;
            String [] delim={";"};
            Network reseau = new Network();
        ...
        public void NetworkInit(Network network)
        {
        ...
        public Link ReadLinkCard(String chaine, String[] delim)
        {
        ...
        public Matrix ReadMatrixFromCsv(String filename)
        {
            string chaine;
            String[] delim = { ";" };
            Matrix matrice = new Matrix();
        ...
        public Trip ReadTripCard(String chaine, String[] delim)
        {
        ...
        public void Assign(Network network, Trip trip, System.IO.StreamWriter fich_od)
        {
...
//                AssignLink(network, id_cout,  trip.demand,trip, fich_od,liens);
        ...
        public void TryReach(Link incoming, Link pivot, List<List<int>> touches,int id_bucket,Network reseau)
        {
            if (((incoming.allowalij==1 && pivot.allowboai== 1) || (pivot.line == incoming.line)) )
            {
                ...
                UpdateProp(incoming,reseau);
                UpdateTime(incoming,reseau);
                AddReached(incoming, touches,id_bucket);
                //incoming.pos = pivot.pos + 1;
            }

        }

        public void AddReached(Link link, List<List<int>> touches,int id_bucket)
        {
            float pu=2;
            int bucket,nb=10000;
            float param =  float.Parse(textBox8.Text);
        ...
        public void OptimizeReduce(Link incoming, Link pivot, List<List<int>> touches,int id_bucket,Network reseau)
        {
        ...
        public void OptimizeAdd(Link incoming, Link pivot, List<List<int>> touches, int id_bucket, Network reseau)
        {
        ...
        public void UpdateM(Link incoming,Network reseau)
        ...
        public void TryOptimize(Link incoming, Link pivot,List<List<int>> touches, int id_bucket,Network reseau)
        {
        ...
        public void ReachInit(Network reseau)
        {
        ...
        public void PrintStrategies(Network reseau, System.IO.StreamWriter fich_res, Trip trip)
        {
            fich_res.WriteLine("\n"+trip.d );
        ...
        public void PrintTimes(Network reseau, System.IO.StreamWriter fich_res,Trip trip )
        {
        ...
        public float ServiceNumberCalc(float hdw)
        {
            if (hdw == 0)
            {
                return 0;
        ...
        public void UpdateProp(Link link,Network reseau)
        {
            link.path_elements.Clear();
            foreach(Strategy s in link.strategies.Values)
            {
        ...
        public void UpdateTime(Link link,Network reseau)
        {
            link.avg_time=0;
            link.avg_cout = 0;
            foreach (Strategy s in link.strategies.Values)
    ...
    public class Network
    {
        ...
        public float cwait = 0, tboa = 0, cboa = 0, cmap = 0;
